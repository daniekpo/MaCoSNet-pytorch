...
class TSS(Dataset):
    def __init__(
        ...
        self.flip_img_A = self.train_data.iloc[:, 3].to_numpy().astype("int")
        self.pair_category = self.train_data.iloc[:, 4].to_numpy().astype("int")
    ...
    def __getitem__(self, idx):
        ...
        image_A, im_size_A = self.get_image(
            self.img_A_names[idx], self.flip_img_A[idx]
        )
        image_B, im_size_B = self.get_image(self.img_B_names[idx])
        image_C, im_size_C = self.get_image(
            self.hash_table[class_label][sampled_idx]
        )
