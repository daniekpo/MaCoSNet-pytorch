...
class PFPascal(Dataset):

    """
        PF-PASCAL training dataset.

        Args:
            csv_file (string): Path to the csv file with image names and
            transformations.
            training_image_path (string): Directory with the images.
            output_size (2-tuple): Desired output size
            transform (callable): Transformation for post-processing the
            training pair (eg. image normalization)
    """
    ...
    def get_image(self, img_name_list, idx, flip):
        img_name = os.path.join(
            self.training_image_path, img_name_list.iloc[idx]
        )
...
class PFPascalVal(Dataset):

    """
    
    PF-PASCAL eval/test dataset.
    ...
    Args:
        ...
        output_size (2-tuple): Desired output size
        transform (callable): Transformation for post-processing the training
        pair (eg. image normalization)
    ...
    def __init__(
        ...
        if mode == "test":
            csv_file = config.PF_PASCAL_TEST_DATA
    ...
    def __getitem__(self, idx):
        ...
        if self.pck_procedure == "pf":
            ...
            point_A_coords[0, 0:N_pts] = (
                point_A_coords[0, 0:N_pts] * 224 / im_size_A[1]
            )
            point_A_coords[1, 0:N_pts] = (
                point_A_coords[1, 0:N_pts] * 224 / im_size_A[0]
            )

            point_B_coords[0, 0:N_pts] = (
                point_B_coords[0, 0:N_pts] * 224 / im_size_B[1]
            )
            point_B_coords[1, 0:N_pts] = (
                point_B_coords[1, 0:N_pts] * 224 / im_size_B[0]
            )
