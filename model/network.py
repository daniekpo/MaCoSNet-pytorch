...
class FeatureCorrelation(torch.nn.Module):
    ...
    def forward(self, feature_A, feature_B):
        ...
        if self.shape == "3D":
            ...
            # reshape features for matrix multiplication
            feature_A = feature_A.view(b, c, h * w).transpose(
                1, 2
            )  # size [b,c,h*w]
...
class FeatureRegression(nn.Module):
    def __init__(
        ...
        for i in range(num_layers):
            ...
            nn_modules.append(
                nn.Conv2d(ch_in, ch_out, kernel_size=k_size, padding=0)
            )
...
class WeakMatchNet(CNNGeometric):
    def __init__(
        ...
        super(WeakMatchNet, self).__init__(
        ...
        if s1_output_dim == 6:
            self.geoTnf = GeometricTnf(
                geometric_model="affine", use_cuda=use_cuda
            )
        else:
        ...
        self.FeatureRegression2 = FeatureRegression(
    ...
    def forward(self, batch, training=False):
        ...
        if not training:
            ...
            if "image_A" in batch:
                ...
                img_A, img_B = batch["source_image"], batch["target_image"]

            feat_A, feat_B = self.get_feature2(img_A, img_B)
            corr_AB = self.get_correlation_1(feat_A, feat_B)
            aff_AB = self.get_affine1(corr_AB)

            """ Thin plate spline (tps) transformation """
            img_Awrp = self.warp_image(img_A, aff_AB)
            f_Awrp = self.get_feature1(img_Awrp)
            corr_Awrp_B = self.get_correlation_1(f_Awrp, feat_B)
            ...
            feat_A, feat_B, f_C = self.get_feature3(img_A, img_B, img_C)

            corr_AB, corr_BA = self.get_correlation_2(feat_A, feat_B)
            corr_BC, corr_CB = self.get_correlation_2(feat_B, f_C)
            corr_CA, corr_AC = self.get_correlation_2(f_C, feat_A)
            ...
            """ Tps """
            img_Awrp_B = self.warp_image(
                img_A, aff_AB
            )  # should better align img_B
            img_Bwrp_A = self.warp_image(
                img_B, aff_BA
            )  # should better align img_A

            img_Bwrp_C = self.warp_image(
                img_B, aff_BC
            )  # should better align img_C
            img_Cwrp_B = self.warp_image(
                img_C, aff_CB
            )  # should better align img_B

            img_Cwrp_A = self.warp_image(
                img_C, aff_CA
            )  # should better align img_A
            img_Awrp_C = self.warp_image(
                img_A, aff_AC
            )  # should better align img_C
            ...
            corr_Awrp_B = self.get_correlation_1(f_Awrp_B, feat_B)
            corr_Bwrp_A = self.get_correlation_1(f_Bwrp_A, feat_A)

            corr_Bwrp_C = self.get_correlation_1(f_Bwrp_C, f_C)
            corr_Cwrp_B = self.get_correlation_1(f_Cwrp_B, feat_B)

            corr_Awrp_C = self.get_correlation_1(f_Awrp_C, f_C)
            corr_Cwrp_A = self.get_correlation_1(f_Cwrp_A, feat_A)
    ...
    def get_correlation_1(self, f_A, f_B):
        corr_AB = self.FeatureCorrelation(
            f_A, f_B
        )  # (h_A x w_A) x h_B x w_B -> for T_AB
        return corr_AB

    def get_correlation_2(self, f_A, f_B):
        corr_AB = self.get_correlation_1(f_A, f_B)
        corr_BA = self.get_correlation_1(f_B, f_A)
        return corr_AB, corr_BA

    def get_affine1(self, corr_AB):
        aff_AB = self.FeatureRegression(
            corr_AB
        )  # warp img_A to align img_B -> T_AB
        return aff_AB
    ...
    if transposed:
        ...
        for y in range(kernel_size):
            for x in range(kernel_size):
                w[y, x] = (1 - abs((x - center) / stride)) * (
                    1 - abs((y - center) / stride)
                )
        layer.weight.data.copy_(
            w.div(in_channel).repeat(out_channel, in_channel, 1, 1)
        )
    else:
