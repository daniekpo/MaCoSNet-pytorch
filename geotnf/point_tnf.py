...
class PointTnf(object):
    ...
    def tpsPointTnf(self, theta, points):
        # points are expected in [B,2,N], where first row is X and second
        # row is Y reshape points for applying Tps transformation
    ...
    def affPointTnf(self, theta, points):
        ...
        warped_points += (
            theta_mat[:, :, 2].unsqueeze(2).expand_as(warped_points)
        )
        return warped_points
...
def PointsToUnitCoords(P, im_size):
    ...
    P_norm = P.clone()
    # normalize Y
    P_norm[:, 0, :] = normalize_axis(
        P[:, 0, :], w.unsqueeze(1).expand_as(P[:, 0, :])
    )
    # normalize X
    P_norm[:, 1, :] = normalize_axis(
        P[:, 1, :], h.unsqueeze(1).expand_as(P[:, 1, :])
    )
    return P_norm
...
def PointsToPixelCoords(P, im_size):
    ...
    P_norm = P.clone()
    # normalize Y
    P_norm[:, 0, :] = unnormalize_axis(
        P[:, 0, :], w.unsqueeze(1).expand_as(P[:, 0, :])
    )
    # normalize X
    P_norm[:, 1, :] = unnormalize_axis(
        P[:, 1, :], h.unsqueeze(1).expand_as(P[:, 1, :])
    )
    return P_norm
